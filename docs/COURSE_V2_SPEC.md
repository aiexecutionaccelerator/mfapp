...
H1 `THE COURSE`. Sub `One lesson a day for 30 days. Watch, reflect, then take today's Mission.` Summary `{n} of 30 complete · {n} Course Reps`. Then a **Today** card (glass, gold border): `DAY {d} · {title}` + `{minutes} min` + button `OPEN TODAY'S LESSON` (d = challengeDay, clamp 1..30; after the challenge, Today card shows "Pick any lesson" instead). Then modules as sections (eyebrow `MODULE {n}` + title), rows: `DAY {day}` small gold label, title, minutes, trigger dot, check if complete. Nothing is locked (no punishment). No points, no Academy link anywhere. Remove the Settings "Academy ↗" row and `ACADEMY_URL`.

### 3.2 Lesson page `/course/[id]`
Back ‹. Eyebrow `DAY {day} · MODULE {module}`. H1 title. **Video**: 16:9 glass frame with `<iframe src="https://www.youtube-nocookie.com/embed/{id}?rel=0&modestbranding=1&playsinline=1" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen loading="lazy" title={title}>`; poster = `https://i.ytimg.com/vi/{id}/hqdefault.jpg` shown until the user taps play (click-to-load facade keeps the page light); lessons without video show no frame. **Body**: markdown renderer (tiny, in-repo: headings, bold, paragraphs, bullet/numbered lists, images `max-w-full rounded-[14px]`, links open in new tab). **Reflect** section (if prompts): each `text` prompt = label + textarea (autosave on blur + 800ms debounce, 4000 max, shows "Saved"), `commit`/`check` = glass row with checkbox. **Today's Mission** card: eyebrow `TODAY'S MISSION`, text `Wear your Scent Trigger and take one small courageous action to honor your commitment to being your best self.` button `START A MISSION` → `/mission/declare?trigger={lesson.trigger ?? ''}&day={day}` (no trigger → Home's three cards, i.e. `/home#select` — simplest: route to `/home`). **Mark complete** primary (→ Course Rep +1, toast `Lesson complete. Course Rep +1.`) then `Completed` state; `Next lesson →` ghost. For `visionSection` lessons also show a link `View my Vivid Vision so far →` (§4).
